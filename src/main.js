  ...
  return {
    ...
    crudsSelectedForFilament() {
      let pluginData = vemto.getPluginData(),
        hasCrudForGeneration = pluginData.cruds.find(
          (crud) => crud && crud.selected
        );

      if (!hasCrudForGeneration) {
        vemto.log.warning(
          "There is no selected CRUD for generating Filament Resources."
        );
        return [];
      }
    ...
    beforeCodeGenerationEnd() {
      ...
      if (vemto.versionIsSmallerThan(phpVersion, "8.0.0")) {
        vemto.log.error(
          "[FILAMENT ERROR] You have a smaller PHP version than required to use the Filament v2 (>= 8.0)"
        );
    ...
    renderFilamentTrait() {
      ...
      vemto.renderTemplate(
        "files/traits/FilamentTrait.vemtl",
        `${basePath}/FilamentTrait.php`,
        options
      );
    },
    ...
    addSelectedCrudsToRepository(cruds) {
      ...
      cruds.forEach((crud) => {
        let crudData = projectCruds.find(
          (projectCrud) => projectCrud.id === crud.id
        );

        if (!crudData) return;

        crudData = this.generatePluginConfigForCrud(
          crudData,
          crud.inputs,
          crud.relationships,
          false
    ...
    resolveCrudRelationships(crud, crudIsFromPluginConfig = true) {
      ...
      relationships.forEach((rel) => {
        ...
        let relationshipIsNotSelected =
          !crudRelationshipData || !crudRelationshipData.selected;

        if (crudIsFromPluginConfig && relationshipIsNotSelected) return;

        let relModelCrud = rel.model.getMainCruds()[0],
          crudModelExistsOnRepository = this.crudRepository.find(
            (crud) => crud.model.id === rel.model.id
          );
        ...
        relModelCrud = this.generatePluginConfigForCrud(
          relModelCrud,
          true,
          {},
          true
    ...
    generateFilamentFiles() {
      ...
      vemto.renderTemplate(
        "files/traits/HasDescendingOrder.vemtl",
        `${basePath}/Traits/HasDescendingOrder.php`,
        {}
      );

      this.crudRepository.forEach((crud) => {
        let crudModelRelationships = this.getAllRelationshipsFromModel(
            crud.model
          ),
          modelRelationshipsManager = this.getCrudModelRelationshipsManager(
            crud,
        ...
        vemto.renderTemplate(
          "files/FilamentResource.vemtl",
          `${basePath}/Resources/${crud.model.name}Resource.php`,
          options
        );
        vemto.renderTemplate(
          "files/pages/Edit.vemtl",
          `${basePath}/Resources/${crud.model.name}Resource/Pages/Edit${crud.model.name}.php`,
          options
        );
        vemto.renderTemplate(
          "files/pages/View.vemtl",
          `${basePath}/Resources/${crud.model.name}Resource/Pages/View${crud.model.name}.php`,
          options
        );
        vemto.renderTemplate(
          "files/pages/List.vemtl",
          `${basePath}/Resources/${crud.model.name}Resource/Pages/List${crud.model.plural}.php`,
          options
        );
        vemto.renderTemplate(
          "files/pages/Create.vemtl",
          `${basePath}/Resources/${crud.model.name}Resource/Pages/Create${crud.model.name}.php`,
          options
        );
        ...
        this.generateRelationshipsManager(
    ...
    generateFilters(crud) {
      ...
      filters.forEach((filter) => {
        if (filter == "DateRange" && crud.model.hasTimestampFields()) {
          vemto.renderTemplate(
            `files/filters/${filter}.vemtl`,
            `${basePath}/${filter}Filter.php`,
            {}
          );
        }
      });
    ...
    generateRelationshipsManager(modelRelationshipsManager, crud, basePath) {
      modelRelationshipsManager.forEach((rel) => {
        let relModelCrud = this.crudRepository.find(
          (crudData) => crudData.model.id === rel.model.id
        );

        if (!relModelCrud) return;

        let relationshipOptions = this.getOptionsForFilamentResource(
          relModelCrud,
          true,
          rel,
          crud.model
        ...
        vemto.renderTemplate(
          "files/ResourceManager.vemtl",
          `${basePath}/Resources/${
            crud.model.name
          }Resource/RelationManagers/${rel.model.plural.case(
            "pascalCase"
          )}RelationManager.php`,
          relationshipOptions
        );
    ...
    getOptionsForFilamentResource(
      crud,
      isRelationManager = false,
      rel = {},
      inverseRelationshipModel = {}
    ) {
      let options = {
        formatAs: "php",
        data: {
          ...
          getRelationshipInputName: (input) =>
            this.getRelationshipInputName(input),
          inputCanBeSearchableIndividually: (input) =>
            this.inputCanBeSearchableIndividually(input),
        },
        modules: [
      ...
      options.data.modelRelationshipsManager =
        this.getCrudModelRelationshipsManager(crud, crudModelRelationships);
    ...
    getCrudModelRelationshipsManager(crud, crudModelRelationships) {
      let crudPluginData = vemto.getPluginData().cruds,
        relationshipsAllowedByFilament = [
          "morphMany",
          "hasMany",
          "belongsToMany",
        ];
      ...
      return crudModelRelationships.filter((relationship) => {
        ...
        let relationshipData = crudPluginData[crud.id].relationships[
          relationship.id
        ]
        ...
        let repositoryHasCrudForRelModel = this.crudRepository.some(
          (crud) => crud.model.id == relationship.model.id
        );
    ...
    getInputsForTable(crud) {
      let textInputs = crud.inputs.filter(
        (input) =>
          !input.isFile() &&
          !input.isJson() &&
          !input.isHidden() &&
    ...
    crudHasTextInputs(crud) {
      return (
        ...
        crud.hasEmailInputs() ||
        crud.hasUrlInputs() ||
        crud.hasPasswordInputs() ||
        crud.hasNumericInputs()
      );
    },
    ...
    getValidationFromInput(input) {
      let inputValidation = input.convertValidationToArrayForTemplate(
          input.validation
        ),
    ...
    inputCanBeSearchable(input) {
      return (
        !input.isDateOrDatetime() &&
        !input.isPassword() &&
        !input.isJson() &&
        !input.isCheckbox() &&
        !input.isForRelationship() &&
        !input.isFileOrImage()
      );
    },
    ...
    inputCanBeSearchableIndividually(input) {
      return (
        input.isText() || input.isEmail() || input.isUrl() || input.isNumeric()
      );
    },
  };
};
