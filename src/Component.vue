<template>
  <div class="w-full">
    <label class="block text-sm font-bold">Filament V3</label>
    <small class="mb-2"
      >Select the CRUDs to generate a Filament V3 Resource</small
    >
    ...
    <div class="mt-5">
      ...
      <div class="form-check mb-3">
        <label class="inline-flex items-center" for="selectAllCruds">
          <input
            ...
            v-model="pluginData.allSelected"
          />
      ...
      <template v-if="pluginData.cruds">
        <div
          class="bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-900 p-2 rounded-md my-3"
          v-for="crud in projectCruds"
          :key="'crud' + crud.id"
        >
          <div class="form-check">
            ...
            >
              <input
                class="form-checkbox"
                type="checkbox"
                v-model="pluginData.cruds[crud.id]['selected']"
                :id="crud.id"
                @change="toggleCrudData(crud)"
              />
          ...
          <div class="form-check mt-1 ml-3">
            <label class="inline-flex items-center">
              <input
                class="form-checkbox"
                type="checkbox"
                v-model="pluginData.cruds[crud.id]['inputs']"
                @change="save"
              />
          ...
          <div
            class="form-check my-1 ml-3"
            v-for="relationship in getAllRelationshipsFromModel(crud.model)"
          ...
          >
            <label class="inline-flex items-center">
              <input
                ...
                type="checkbox"
                v-model="
                  pluginData.cruds[crud.id]['relationships'][relationship.id]
                    .selected
                "
                @change="save"
              />
              <span class="ml-2 text-gray-800 dark:text-gray-300">{{
                `${relationship.type.case(
                  "pascalCase"
                )} (${relationship.name.case("pascalCase")})`
              }}</span>
            </label>
          </div>
        </div>
  ...
  methods: {
    checkNewModelRelationships() {
      this.projectCruds.forEach((crud) => {
        let crudPluginData = this.pluginData.cruds.find(
          (crudData) => crudData && crudData.id === crud.id
        );
        ...
        crudModelRelationships.forEach((rel) => {
          if (!crudPluginData.relationships[rel.id]) {
            this.$set(this.pluginData.cruds[crud.id].relationships, rel.id, {
              selected: false,
            });
          }
        });
